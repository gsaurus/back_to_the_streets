...
public class InputButtonCondition: BoolCondition<GameEntityModel>{
...
// Condition based on input button
public static class InputButtonOperations{

	public static void ConsumePress(AnimationModel model, uint buttonId){
		Model inputModel = InputConditionsHelper.GetInputModel(model);
	...
	public static void ConsumeRelease(AnimationModel model, uint buttonId){
		Model inputModel = InputConditionsHelper.GetInputModel(model);
		if (inputModel == null) return;
		GameEntityInputProvider inputController = inputModel.Controller() as GameEntityInputProvider;
		if (inputController == null) return;
