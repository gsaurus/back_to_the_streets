...
public static class CharacterConditionsBuilder {
	...
	private delegate GenericTriggerCondition<GameEntityModel> BuilderAction(Storage.GenericParameter param, out int keyFrame, Storage.CharacterAnimation animation);
	...
	public static GenericTriggerCondition<GameEntityModel> Build(Storage.Character charData, int[] conditionIds, out int keyFrame, Storage.CharacterAnimation animation){
		List<GenericTriggerCondition<GameEntityModel>> conditions = new List<GenericTriggerCondition<GameEntityModel>>(conditionIds.Length);
		GenericTriggerCondition<GameEntityModel> condition;
		...
		if (conditions.Count > 0) {
			...
			return new ConditionsList<GameEntityModel>(conditions);
		}
		return null;
	}
	...
	private static GenericTriggerCondition<GameEntityModel> BuildFromParameter(Storage.GenericParameter parameter, out int keyFrame, Storage.CharacterAnimation animation){
		keyFrame = invalidKeyframe;
		GenericTriggerCondition<GameEntityModel> condition;
		...
		if (callIndex < builderActions.Length) {
			...
			if ( IsNegated(parameter) ) {
				condition = new NegateCondition<GameEntityModel>(condition);
			}
			return condition;
		}
...
#region Conditions
	...
	// frame = 4
	private static GenericTriggerCondition<GameEntityModel> BuildKeyFrame(Storage.GenericParameter parameter, out int keyFrame, Storage.CharacterAnimation animation){
